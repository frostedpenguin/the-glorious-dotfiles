...
screen.connect_signal("request::desktop_decoration", function(s)
	...
	local toggle_music_box = function(type)
		...
		if music_box.visible then
			...
			if type == "keyboard" then
				...
				awful.placement.top_right(music_box, {
					margins = {
						top = dpi(5),
	...
	awesome.connect_signal("widget::music", function(type)
		toggle_music_box(type)
	end)

	s.backdrop_music:buttons(awful.util.table.join(awful.button({}, 1, nil, function()
		toggle_music_box()
	end)))
end)
